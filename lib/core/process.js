...
let log = new Log(process.env.LOG_LEVEL || 'ERROR', 'Process')
...
class Process {
  ...
  create(command, args, callbacks) {
    log.debug('execFile %s %s', command, args.join(' '))
    function dummy() { }
    let object = this
    return new Bluebird((resolve, reject) => {
      ...
      log.debug('created %d', object.pid)
  ...
  status() {
    if(!this.pid) {
      log.error('no pid associated to check status')
  ...
  stop() {
    ...
    return new Bluebird(resolve => {
      ...
      if(!object.proc) {
        log.error('cannot kill external process')

...
class Job extends JobBase {
  ...
  static createMultiple(url, browsers, capabilities, runId, isNative) {
    ...
    return Bluebird.all(bRetAll.map(bRet => {
      let
        vRet = Values.js(bRet.browser || { }, bRet.capabilities),
        job = new Job(bRet.url, vRet.browser, vRet.capabilities, bRet.serverId)
      return startJob(job)
      .catch(() => {
        return job
      })
  ...
  stop(passed) {
    // Another hack for crossbrowsertesting - the live test does have a
    // test_score field; but there is no way to set it through livetests
    // api. However, using the id as a selenium test id and using the
    // selenium API to set score works even for the live test.
    return writeRequest(VARS.baseUrl.replace(/livetests/, 'selenium') + '/' + this.id, 'PUT', {
      action: 'set_score',
      score: false !== passed ? 'pass' : 'fail'
    })
    .then(() => {
...
function startJob(job) {
  ...
  return writeRequest(VARS.baseUrl, 'POST', getLiveTestParams(job))
  .then(response => {

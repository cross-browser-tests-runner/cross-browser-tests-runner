...
function termCallbacks(worker) {
  return {
    ...
    onresponse: function(response) {
      log.debug('%s delete call response status %d', worker.id, response.statusCode)
    }
  }
}
...
function safeTerminate(worker, callbacks) {
  return waitForRunningRetries(worker)
  .then(() => {
    return safeTerminateRetries(worker, callbacks)
  })
  .catch(() => {
    return safeTerminateRetries(worker, callbacks)
  })
...
function waitForRunningRetries(worker) {
  let max = 6, minTimeout = 500, factor = 2
  const check = () => {
    return worker.status()
    .then(status => {
      if('running' === status || 'terminated' === status) {
        log.debug('worker %s status is "%s", wait for non-queue status is over', worker.id, status)
        return true
      }
      log.debug('worker %s status is "%s", waiting till it becomes "running"', worker.id, status)
      throw new Error('Platforms.BrowserStack.Worker: worker status is not "running" yet')
    })
  }
  return retry(check, { retries: max, minTimeout: minTimeout, factor: factor })
}

function safeTerminateRetries(worker, callbacks) {
  ...
  const check = () => {
    ...
    .then(status => {
      if('terminated' === status) {
        log.debug('worker %s is terminated', worker.id)
        return true
      }
      log.debug('worker %s not terminated yet, with status reported "%s", retrying to terminate', worker.id, status)

...
class Job extends JobBase {
  ...
  static create(url, browser, capabilities, runId, isNative) {
    let bRes = JobBase.create(url, browser, capabilities, runId, isNative)
    let options = Object.assign(bRes.browser || { }, bRes.capabilities || { })
    options.url = bRes.url
    return startJob(new Job(options, bRes.serverId))
  }

  static createMultiple(url, browsers, capabilities, runId, isNative) {
    if(!browsers || !browsers.length) {
      throw new Error('Platforms.BrowserStack.Job: no browsers specified for createMultiple')
    }
    ...
    bResAll.forEach(bRes => {
      let options = Object.assign(bRes.browser, bRes.capabilities)
      options.url = bRes.url
      promises.push(startJob(new Job(options, bRes.serverId)))
    })

...
exports.serverArgs = (allowedOptions, help) => {
  ...
  return (require('minimist')(process.argv.slice(2), {
    string: ['config'],
    boolean: ['help', 'native-runner', 'errors-only', 'omit-traces', 'error-reports-only', 'omit-report-traces'],
    alias: {config: 'c', help: 'h', 'native-runner': 'n', 'errors-only': 'e', 'omit-traces': 'o', 'error-reports-only': 'E', 'omit-report-traces': 'O' },
    unknown: opt => {
      return onUnknownOpt(-1 !== allowedOptions.indexOf(opt), opt, help)
    }
  }))
...
exports.serverHelp = () => {
  console.log(
    '\n' +
    path.basename(process.argv[1]) +
    ' [--help|-h] [--config|-c <config-file>] [--native-runner|-n] [--errors-only|-e] [--omit-traces|-o] [--error-reports-only|-E] [--omit-report-traces|-O]\n\n' +
    'Defaults:\n' +
    ' config              cbtr.json in project root, or CBTR_SETTINGS env var\n' +
    ' native-runner       false\n' +
    ' errors-only         false\n' +

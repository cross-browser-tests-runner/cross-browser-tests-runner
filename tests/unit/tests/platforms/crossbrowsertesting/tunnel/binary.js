...
var
  ...
  fs = require('fs'),
  nock = require('nock'),
  binary = require('./../../../../../../lib/platforms/crossbrowsertesting/tunnel/binary'),
...
describe('Binary', function() {
  ...
  describe('fetch', function() {
    ...
    it('should retry if HTTP requests fail and eventually bail out once retries are exhausted', function() {
      var url = BinaryVars.url.replace(/^.*github\.com/, '')
      nock('https://github.com').get(url).times(5).replyWithError('simulating request failure')
      binary = new Binary()
      return binary.fetch()
      .catch(err => {

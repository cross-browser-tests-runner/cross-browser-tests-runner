...
describe('saucelabs.js', function() {

  this.timeout(0)

  it('should update saucelabs-conf.json', function() {
    var proc = new Process(),
      configFile = path.resolve(process.cwd(), 'conf/saucelabs-conf.json'),
      lastMtime, currMtime
    return fs.statAsync(configFile)
    .then(stats => {
      lastMtime = stats.mtime.getTime()
      return proc.create('node',
        utils.nodeProcCoverageArgs('bin/utils/conf/browsers/saucelabs.js'), {

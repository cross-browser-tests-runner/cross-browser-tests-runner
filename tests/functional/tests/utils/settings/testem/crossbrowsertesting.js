...
describe('CrossBrowserTesting', function() {
  ...
  it('should fail if an unknown command line option is provided', function() {
    ...
    return proc.create('node',
      utils.nodeProcCoverageArgs('bin/utils/settings/testem/crossbrowsertesting.js', ['--unknown']), {
  ...
  it('should print help if "--help" command line option is provided', function() {
    var proc = new Process(), out = ''
    return proc.create('node',
      utils.nodeProcCoverageArgs('bin/utils/settings/testem/crossbrowsertesting.js', ['--help']), {
      onstdout: function(stdout) {
        out += stdout
      }
    })
    .then(() => {
      expect(out).to.contain("crossbrowsertesting.js [--help|-h] [--input|-i <cbtr-settings-file>] [--output|-o <testem-settings-file>]")
      return true
    })
    .catch(err => {
      utils.log.error('error: ', err)
  ...
  it('should successfully create testem.json for a valid cbtr.json input file', function() {
    ...
    .then(() => {
      ...
      return proc2.create('node',
        utils.nodeProcCoverageArgs('bin/utils/settings/testem/crossbrowsertesting.js'), {
        onstdout: function(stdout) {
